...
func (s *SegmentSpanImpl) End() {
	...
	p := s.tracer().ProfileManager
	p.CheckProfileValue(segmentID, id, s.StartTime(), s.EndTime())
...
func (rs *RootSegmentSpan) End() {
	...
	p := rs.tracer().ProfileManager
	p.CheckProfileValue(segmentId, spanId, rs.StartTime(), rs.EndTime())
	if !rs.InAsyncMode {
		rs.end0()
	}
}
...
func newSegmentRoot(segmentSpan *SegmentSpanImpl) *RootSegmentSpan {
	...
	go func() {
		...
		s.tracer().ProfileManager.EndProfiling(s.SegmentID)
